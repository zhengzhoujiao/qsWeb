<template>
  <div>
    ...
    >
      <el-form>
        <el-form-item v-for="(item,index) in dataSource" :key="index" label="导航名称" :label-width="formLabelWidth">
...
export default {
  data() {
    return {
      ...
      dataSource: this.$store.getters.getcomputedData("theNavbar2").main.data,
      num: this.$store.getters.getcomputedData("theNavbar2").main.data.length,
  ...
  computed: {
    getDate() {
      return this.$store.getters.getcomputedData("theNavbar2").main.data;
    },
  },
  watch: {
    getDate(val) {
